 ...
 *
 * Configurable via hoard.curfew.* in ~/.pi/agent/settings.json:
...
function getCurfewSettings(): { enabled: boolean; startHour: number; endHour: number } {
	return {
		enabled: readHoardSetting<boolean>("curfew.enabled", true),
		startHour: readHoardSetting<number>("curfew.startHour", 0),
		endHour: readHoardSetting<number>("curfew.endHour", 6),
	};
}
...
export default function dragonCurfew(pi: ExtensionAPI): void {
	...
	pi.registerCommand("curfew", {
		...
		handler: async (_args, ctx) => {
			...
			if (!enabled) {
				ctx.ui.notify(`🐉 Dragon Curfew: disabled (hours would be ${label})`, "info");
				return;
			}

			if (!active) {
				const nextLabel = `Curfew starts tonight at ${formatHour(startHour)}`;
				ctx.ui.notify(`🌅 Dragon Curfew: inactive — ${localTime} — ${nextLabel}`, "info");
				return;
			}
