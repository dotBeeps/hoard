...
- **Puppy Mode** (`plan`): Read-only planning. Safe bash commands (`cat`, `diff`, `git status`) and read-only tools auto-allow. Mutating tools (file writes, git mutations, package installs) require permission. Designed for analysis and planning before implementation.
- **Dragon Mode** (`dragon`): All tools allowed, no prompting. Full implementation mode.
