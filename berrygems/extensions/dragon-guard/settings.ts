/**
 * Dragon Guard settings — reads from hoard.guard namespace.
 *
...
export const GUARD_DOG_ALLOWED_TOOLS = readHoardSetting<string[]>(
	"guard.dogAllowedTools",
	["read", "ls", "find", "grep", "questionnaire"],
);

/** Default tool allowlist for Puppy Mode (read-only planning). */
export const GUARD_PUPPY_ALLOWED_TOOLS = readHoardSetting<string[]>(
...
let _autoDetect = readHoardSetting<boolean>("guard.autoDetect", true);
let _complexityThreshold = readHoardSetting<number>("guard.complexityThreshold", 4);
let _llmSummaries = readHoardSetting<boolean>("guard.llmSummaries", true);
...
export { writeProjectHoardSetting as writeProjectSetting };
