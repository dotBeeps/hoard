...
function loadSettings(): WebSearchSettings {
	const raw = readHoardSetting("websearch", {}) as Partial<WebSearchSettings>;
	return {
...
async function searchZai(
	...
	if (toolCalls) {
		const webSearchCall = toolCalls.find((t) => (t as Record<string, unknown>).type === "web_search");
		if (webSearchCall) {
			const webSearch = webSearchCall.web_search as Record<string, unknown> | undefined;
			const searchResults = (webSearch?.search_results as Array<Record<string, unknown>>) ?? [];
...
async function getZaiApiKey(): Promise<string | null> {
	try {
		...
		return typeof entry === "string" ? entry : (entry as Record<string, unknown>).key as string ?? null;
	} catch {
		return null;
	}
}
...
export default function (pi: ExtensionAPI) {
	...
	(pi.registerTool as any)({
		name: "web_search",
		description: "Search the web for current information. Use when you need up-to-date facts, recent events, documentation, or anything not in your training data.",
		params: SearchParams,
		...
		async execute(_id: string, params: any, _signal: any, onUpdate: any) {
			onUpdate?.({ type: "progress", detail: `Searching for: ${params.query}` });
				...
				switch (settings.backend) {
					case "brave": {
						if (!settings.braveApiKey) {
							return { output: "Brave search requires `hoard.websearch.braveApiKey` in settings." };
						}
						results = await searchBrave(params.query, settings.braveApiKey, settings.maxResults);
						break;
					}

					case "searxng": {
						if (!settings.searxngUrl) {
							return { output: "SearXNG requires `hoard.websearch.searxngUrl` in settings." };
						}
						results = await searchSearxng(params.query, settings.searxngUrl, settings.maxResults);
						break;
					}
						...
						if (!apiKey) {
							...
							return {
								output: [
									...
									"- Switch backend: `hoard.websearch.backend` = \"brave\" or \"searxng\"",
								].join("\n"),
							};
						}
						const zaiModel = settings.zaiModel ?? "glm-4-flash";
						onUpdate?.({ type: "progress", detail: `Querying zai (${zaiModel})...` });
						results = await searchZai(params.query, apiKey, zaiModel, settings.maxResults);
						break;
					}
				}

				onUpdate?.({ type: "progress", detail: `Found ${results.length} results` });
				return {
					output: formatResults(results),
					details: { query: params.query, count: results.length, source: settings.backend },
				};
			} catch (err) {
