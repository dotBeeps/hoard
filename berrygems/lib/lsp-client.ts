/**
 * Minimal LSP Client — JSON-RPC over stdio to typescript-language-server.
 *
 * Handles the LSP framing protocol (Content-Length headers), initialization
 * handshake, and diagnostic streaming. Designed for embedding in pi extensions
 * where we need live type errors without running tsc.
 *
...
export class LspClient extends EventEmitter {
	...
	openFile(filePath: string): void {
		...
		let text: string;
		try {
			text = readFileSync(resolve(this.cwd, filePath), "utf-8");
		} catch {
			return;
		}
	...
	/** Open all TypeScript files under a directory. */
	openDirectory(dir: string, extensions = [".ts"]): void {
		const walk = (d: string) => {
			try {
	...
	private handleDiagnostics(params: any): void {
		...
		this.emit("diagnostics", { uri, diagnostics } satisfies LspDiagnosticEvent);
	}
}
